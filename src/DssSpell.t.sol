...
contract DssSpellTest is DssSpellTestBase {
    ...
    function testCollateralIntegrations() public { // make private to disable
        ...
        checkIlkIntegration(
            "WPLS-A",
            GemJoinAbstract(addr.addr("MCD_JOIN_WPLS_A")),
            ClipAbstract(addr.addr("MCD_CLIP_WPLS_A")),
            addr.addr("PIP_WPLS"),
            true, /* _isOSM */
    ...
    function testIlkClipper() public { // make private to disable
        ...
        checkIlkClipper(
            ...
            GemJoinAbstract(addr.addr("MCD_JOIN_WPLS_A")),
            ClipAbstract(addr.addr("MCD_CLIP_WPLS_A")),
            addr.addr("MCD_CLIP_CALC_WPLS_A"),
            OsmAbstract(addr.addr("PIP_WPLS")),
            5_000 * WAD
        );
    ...
    function testNewChainlogValues() public { // make private to disable
        ...
        checkChainlogKey("WPLS");
        checkChainlogKey("PIP_WPLS");
        checkChainlogKey("MCD_JOIN_WPLS_A");
        checkChainlogKey("MCD_CLIP_WPLS_A");
        checkChainlogKey("MCD_CLIP_CALC_WPLS_A");
    ...
    function testNewIlkRegistryValues() public { // make private to disable
        ...
        // WPLS-A
        assertEq(reg.pos("WPLS-A"),    56);
        assertEq(reg.join("WPLS-A"),   addr.addr("MCD_JOIN_WPLS_A"));
        assertEq(reg.gem("WPLS-A"),    addr.addr("WPLS"));
        assertEq(reg.dec("WPLS-A"),    GemAbstract(addr.addr("WPLS")).decimals());
        assertEq(reg.class("WPLS-A"),  1);
        assertEq(reg.pip("WPLS-A"),    addr.addr("PIP_WPLS"));
        assertEq(reg.name("WPLS-A"),   "Wrapped Pulse");
        assertEq(reg.symbol("WPLS-A"), GemAbstract(addr.addr("WPLS")).symbol());
